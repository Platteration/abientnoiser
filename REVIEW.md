...
4. Open, by decision: no `npm audit` step. Unactioned advisories only turn CI red;
   ...
   (`claude/ambient-music-looper-hvttuo`), which is not this one, so here they are no
   channel at all. `ci.yml` has an `audit` job of its own running
   `npm audit --omit=dev --audit-level=high`, rather than the step after `npm ci` in the
   workflow below; it has nothing to audit yet, Playwright being the one dependency and a
   development one.
